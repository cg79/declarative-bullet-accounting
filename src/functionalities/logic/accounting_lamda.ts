...
export function executeAccountingLamda(
  ...
  const functionResult = CodeExecutionService.executeDelta(delta, param);
  return functionResult.response;
  // return accounting_lamda(param);
}

export function accounting_lamda(obj) {
  try {
    console.log("--");
    console.log(obj);
    console.log("+++++");
    const [{ previous_casa, country_taxes, salaries }, { accountingRequest }] =
      obj;

    console.log("country_taxes", JSON.stringify(country_taxes, null, 2));
    const tvaRecord = country_taxes.find(
      (el) =>
        el.nume === "TVA" &&
        Date.parse(el.date) < Date.parse(accountingRequest.datetime)
    ) || {
      value: 0,
    };
    const dividendeRecord = country_taxes.find(
      (el) =>
        el.nume === "Dividende" &&
        Date.parse(el.date) < Date.parse(accountingRequest.datetime)
    ) || {
    ...
    const profitRecord = country_taxes.find(
      (el) =>
        el.nume === "TaxaProfit" &&
        Date.parse(el.date) < Date.parse(accountingRequest.datetime)
    ) || {
    ...
    const pensieRecord = country_taxes.find(
      (el) =>
        el.nume === "TaxaPensie" &&
        Date.parse(el.date) < Date.parse(accountingRequest.datetime)
    ) || {
    ...
    const sanatateRecord = country_taxes.find(
      (el) =>
        el.nume === "TaxaSanatate" &&
        Date.parse(el.date) < Date.parse(accountingRequest.datetime)
    ) || {
      value: 0.1,
    };

    const asigurareRecord = country_taxes.find(
      (el) =>
        el.nume === "TaxaAsigurareMunca" &&
        Date.parse(el.date) < Date.parse(accountingRequest.datetime)
    ) || {
      value: 0.0225,
    };
    const salarRecord = salaries.find(
      (el) => Date.parse(el.date) < Date.parse(accountingRequest.datetime)
    ) || {
      value: 0,
      scutire_impozit: 0,
    };
    ...
    const { value = 0, operationid = 0 } = accountingRequest;
    ...
    switch (Number(operationid)) {
      case 1: {
        ...
        newConta.taxe.taxa_profit =
          (newConta.taxe.taxa_profit || 0) + taxa_profit;

        newConta.taxeTrezorerie =
          (newConta.taxeTrezorerie || 0) + newConta.taxe.taxa_profit;

        newConta.taxe.total = newConta.taxe.total + newConta.taxeTrezorerie;
      ...
      case 8: {
        ...
        newConta.taxe.total = newConta.taxe.total - value;
        newConta.taxeTrezorerie =
          (newConta.taxeTrezorerie || 0) - newConta.taxe.dividende;
      ...
      case 9: {
        ...
        newConta.casa.firma = newConta.casa.firma - value;
        newConta.casa.cont_personal =
          (newConta.casa.cont_personal || 0) + value;
      ...
      case 12: {
        ...
        console.log(previous);
        newConta.casa.firma = newConta.casa.firma - value;
        newConta.plati.comision_intrare =
          previous.plati.comision_intrare + value;
